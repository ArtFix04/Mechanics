...
public class FloorPlanningOptimized extends AbstractSimulation {
    PlotFrame frame = new PlotFrame("x", "y", "Optimized Floor-planning Problem");
    ...
    double cellSize = 5.0;
    double areaWidth = 25, areaHeight = 25;
    ...
    class Particle implements Drawable {
        ...
        void updatePosition() {
            ...
            x = Math.min(Math.max(radius, x), areaWidth - radius);
            y = Math.min(Math.max(radius, y), areaHeight - radius);
            circle.setXY(x, y);
        }
    ...
    public static void main(String[] args) {
        SimulationControl.createApp(new FloorPlanningOptimized());
    }
}
